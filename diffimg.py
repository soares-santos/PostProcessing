...
def read_data_files(path='.',filenamepattern='*.dat'):
	...
	OBSCCDNUM  = np.zeros(0,dtype=int)
		...
		OBSCCDNUM = np.append(OBSCCDNUM,float(line[17]))
                if thisFAKE == 1 : 
			SIMMAG = np.append(SIMMAG,float(line[15]))
		else:
